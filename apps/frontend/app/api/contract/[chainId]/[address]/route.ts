...
const etherscanUrl = (chainId: number, address: Hex) => {
  return `${_.get(ETHERSCAN_API_URLS, chainId)}?module=contract&action=getsourcecode&address=${address}&apikey=${_.get(
    ETHERSCAN_KEYS,
    chainId,
  )}`;
};
...
export async function GET(request: Request, { params }: { params: { chainId: string; address: Hex } }) {
  const { chainId: initialChainId, address } = _.pick(params, ['chainId', 'address']);
  ...
  try {
    ...
    const returnData = _.mapKeys(_.get(data, 'result[0]'), (value: unknown, key: string) => _.camelCase(key));
