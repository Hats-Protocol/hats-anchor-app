    ...
    <div className='flex flex-col'>
      {STEPS.map((step, index) => (
        <div key={step.id}>
          <Link href={`/councils/new/${step.id}?draftId=${draftId}`}>
            <div className='flex gap-4'>
              <div className='flex flex-col items-center'>
                ...
                {/* Vertical line */}
                {index < STEPS.length - 1 && step.id !== 'selection' && (
                  <div
          ...
          {/* Sub-steps */}
          {step.id === 'selection' && step.subSteps && (
            <div className='my-3 ml-[23px]'>
              {step.subSteps.map((subStep, subIndex) => (
                <div
                  key={subStep.id}
                  className='flex items-center gap-3 border-l-[2px] border-l-blue-500'
                >
                  <div className='my-1 ml-4 flex h-6 w-6 items-center justify-center rounded-full bg-white'>
