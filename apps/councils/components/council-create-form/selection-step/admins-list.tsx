...
export function AdminsList({ name, admins, form }: AdminsListProps) {
  ...
  return (
    <>
      <div className='space-y-4'>
        {admins.map((admin) => (
          <AdminCard key={admin.id} admin={admin} onRemove={handleRemove} onEdit={() => handleEdit(admin)} />
        ))}
      </div>

      <AddAdminModal isOpen={isModalOpen} onClose={handleModalClose} form={form} editingAdmin={editingAdmin} />
    </>
  );
}
