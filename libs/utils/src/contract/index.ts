...
export const checkTransactionStatus = async (transactions: Transaction[]) => {
  ...
  const transactionPromises = pendingTransactions.map(async (tx: Transaction) => {
...
export const extendWearers = (wearers: HatWearer[] | undefined, wearersInfo: HatWearer[] | undefined): HatWearer[] =>
  _.compact(
...
export const extendControllers = (controller: Hex | undefined, controllersInfo: HatWearer[] | undefined) => {
  const controllerInfo = _.find(controllersInfo, { id: _.toLower(controller) });

  return controllerInfo as HatWearer;
};

export const fetchContractData = async (chainId: SupportedChains | undefined, address: Hex | undefined) =>
