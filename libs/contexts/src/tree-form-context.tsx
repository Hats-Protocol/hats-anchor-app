...
export interface TreeFormContext {
  ...
  handleNodeCollapsedOrExpanded: ((nodeIdIp: string, expanded: boolean) => void) | undefined;
...
const TreeFormContextContent = ({ children }: { children: ReactNode }) => {
  ...
  const [selectedOption, setSelectedOption] = useState<string | undefined>('wearers');
  ...
  const [storedData, setStoredData] = useLocalStorage<Partial<FormData>[]>(localStorageKey, []);
  ...
  const linkedHatIds: Hex[] = useMemo(() => {
    const { linkedToHat, parentOfTrees } = _.pick(treeData, ['linkedToHat', 'parentOfTrees']);
    return _.compact(_.concat(_.map(parentOfTrees, 'hats[0].id'), _.get(linkedToHat, 'id')));
  }, [treeData]);
  ...
  const draftHats = useMemo(
    () =>
      _.reject(
        orgChartHats,
        (hat: AppHat) =>
          _.includes(_.map(onchainHats, 'id'), _.get(hat, 'id')) || _.isEmpty(_.reject(hat, ['id', 'parentId'])),
      ),
  ...
  const transformTree = useCallback(
    (tree: any, includeInactive = false) => {
      return _.chain(tree)
        .filter((hat: any) => includeInactive || !_.includes(inactiveHats, _.get(hat, 'id')))
        .map((hat: any) => {
          if (editMode) {
        ...
        .filter((hat: any) => !editMode || hat.id?.startsWith(treeData?.id || ''))
        .value();
    },
  ...
  const treeToDisplay: AppHat[] | undefined = useMemo(
    () => (editMode ? transformTree(filteredTree, true) : filteredTree) as AppHat[] | undefined,
    [editMode, filteredTree, transformTree],
  );

  const treeToDisplayWithInactiveHats = useMemo(() => transformTree(orgChartTree, true), [transformTree, orgChartTree]);

  // *********************
  // * TOP HAT
  // *********************

  const topHat: AppHat | undefined = useMemo(() => _.first(orgChartTree), [orgChartTree]);
  const topHatDetails = useMemo(() => _.get(topHat, 'detailsObject.data'), [topHat]);
    ...
    (nodeIdIp: string, expanded: boolean) => {
      ...
      if (Array.isArray(collapsed)) {
        ...
        updateCollapsedQueryParams(
          expanded ? _.reject(collapsed, (id) => id === nodeIdIp) : _.uniq(_.concat(collapsed, [nodeIdIp])),
        );
        return;
      }
      if (typeof collapsed === 'string') {
        // existing query param is a single collapsed node
        updateCollapsedQueryParams(expanded ? _.reject(collapsed, nodeIdIp) : _.uniq(_.concat(collapsed, nodeIdIp)));
        return;
      }
      if (storedConfig.collapsed && !_.isEmpty(storedConfig.collapsed)) {
        // no query params but there are collapsed nodes in local storage
        if (expanded) {
          updateCollapsedQueryParams(_.uniq(_.concat(storedConfig.collapsed, nodeIdIp)));
          return;
        }

        // update the query params including the collapsed nodes from local storage
        if (_.includes(storedConfig.collapsed, nodeIdIp)) {
          updateCollapsedQueryParams(_.reject(storedConfig.collapsed, (id) => id === nodeIdIp));
          return;
        }
      }
  ...
  const enterEditMode = useCallback(() => {
    const localDraftHats = _.reject(
      ...
      (hat: Partial<FormData>) =>
        _.includes(_.map(onchainHats, 'id'), hat.id) || _.isEmpty(_.reject(hat, ['id', 'parentId'])),
    );
  ...
  const importHats = useCallback(
    ...
    (hats: any[]) => {
      const translateImageUrl = _.map(hats, (hat: any) => {
        ...
        const imageUrl = hat.imageUri ? { imageUrl: ipfsUrl(hat.imageUri?.slice(7)) } : {};
        return {
          ...hat,
          ...imageUrl,
        };
      ...
      const localDraftHats = _.reject(
        translateImageUrl,
        (hat: AppHat) => _.includes(_.map(onchainHats, 'id'), hat.id) || _.isEmpty(_.reject(hat, ['id', 'parentId'])),
      );
      if (!_.isEmpty(localDraftHats) && chainId && treeId) {
        const drafts = translateDrafts({
          chainId,
  ...
  const resetTree = useCallback(() => {
    ...
    onCloseTreeDrawer?.();
    handleExpandAll();
  }, [onchainHats, setStoredData, onCloseHatDrawer, setSelectedOption, onCloseTreeDrawer, handleExpandAll]);
