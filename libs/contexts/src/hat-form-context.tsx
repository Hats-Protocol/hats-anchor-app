...
import { useDebounce, useHatSnapshotRoles, useToast } from 'hooks';
import { filter, find, includes, isEmpty, keys, omit, pick, pickBy, reject, remove } from 'lodash';
...
import { useSelectedHat } from './selected-hat-context';
...
export const HatFormContextProvider = ({ children }: { children: ReactNode }) => {
  const {
    chainId,
    orgChartTree,
    storedData,
  ...
  const { modulesAuthorities, isLoading: isLoadingModulesAuthorities } = useAncillaryModules({
    id: selectedHat?.id,
    chainId,
    editMode: false,
    tree: orgChartTree,
  ...
  //   },
  // );
  const { data: snapshotRoles, isLoading: isLoadingSnapshotRoles } = useHatSnapshotRoles({
    hatId: selectedHat?.id,
    spaces: snapshotData,
    chainId,
  });
  ...
  const formEligibility = useDebounce<Hex | undefined>(watch('eligibility', FALLBACK_ADDRESS));
  const formEligibilityInput = watch('eligibility-input', '');
  const formToggle = useDebounce<Hex | undefined>(watch('toggle', FALLBACK_ADDRESS));
  ...
  const { maxSupply, eligibility, toggle, mutable, imageUrl, details } = pick(selectedOnchainHat, [
    'maxSupply',
    'eligibility',
    'toggle',
    'mutable',
  ...
  useEffect(() => {
    ...
    const initialFormValues = () => {
      ...
      if (matchingHat && !isEmpty(remove(keys(matchingHat), (key: string) => key === 'id'))) {
        formValues = {
          ...defaultFormValues,
          ...matchingHat,
        };
  ...
  const handleSave = useCallback(
    (sendToast: boolean = true) => {
      ...
      const dirtyFormValues = pickBy(debouncedFormValues, (__: unknown, key: FormFieldKeys) =>
        includes(dirtyValues, key),
      );
    ...
    [selectedHat?.id, setStoredData, storedData, toast, debouncedFormValues, defaultFormValues, reset],
  );
  ...
  const handleClearChanges = useCallback(() => {
    ...
    onOpenTreeDrawer?.();
    onCloseHatDrawer?.();
  }, [onCloseHatDrawer, onOpenTreeDrawer, selectedHat, setStoredData, storedData]);
  ...
  return <HatFormContext.Provider value={returnValue}>{children}</HatFormContext.Provider>;
};
