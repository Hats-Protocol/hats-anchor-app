 ...
 */
export const mapWithChainId = (array: object[] | undefined, chainId: number): object[] =>
  map(array, (obj: object) => ({ ...obj, chainId }));
 ...
 */
export function createHierarchy(data: InputObject[], currentHatId?: Hex): Hierarchy {
  ...
  const currentHierarchy: Hierarchy = {
    id: currentHat.id,
    parentId: (currentHat.id === currentHat.parentId ? null : currentHat.parentId) as Hex,
  };
  ...
  if (currentHat.parentId !== currentHat.id) {
    siblings = filter(
      ...
      (hat: Partial<AppHat>) => hat.parentId === currentHat.parentId && hat.id !== hat.parentId,
    );
  }

  const sortedSiblings = sortBy(siblings, (sibling: AppHat) => BigInt(sibling.id));
  ...
  const children = sortBy(
    filter(data, (h: AppHat) => h.parentId === currentHatId && h.id !== currentHatId),
    'id',
  );
