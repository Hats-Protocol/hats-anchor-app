...
const useSubscriptionClaim = ({ moduleParameters, chainId, setStatus, handlePendingTx }: UseSubscriptionClaimProps) => {
  ...
  const disableClaim =
    !lockAddress || !keyPrice || !currencyContract || allowance < keyPrice || tokenBalance < keyPrice;

  let disableReason = '';
  if (!isUndefined(tokenBalance) && !isUndefined(keyPrice) && tokenBalance < keyPrice) {
    disableReason = `Not enough balance: requires ${price} ${symbol}`;
  }
  if (!isUndefined(allowance) && !isUndefined(keyPrice) && allowance < keyPrice) {
  ...
  return {
    ...
    disableClaim,
    disableReason,
    hasAllowance: !isUndefined(allowance) && !isUndefined(keyPrice) && allowance >= keyPrice,
  };
};
