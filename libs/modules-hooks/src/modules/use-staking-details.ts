...
const fetchStakingDetails = async ({
  ...
  const stakeBalanceDisplay = stakeBalanceRaw ? formatUnits(stakeBalanceRaw, tokenDetails?.decimals || 18) : '0';
...
const useStakingDetails = ({
  ...
  return useQuery({
    ...
    staleTime: Infinity,
    enabled: !!moduleDetails && !!moduleParameters && !!tokenAddressParam?.value && !!chainId,
  });
};

export { useStakingDetails };
