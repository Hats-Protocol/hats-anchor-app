'use client';
import { Avatar, Heading, HStack, IconButton, Skeleton, Stack, Text } from '@chakra-ui/react';
...
const WearerInfo = () => {
  ...
  const wearerAddress = _.get(parsedPathname, _.subtract(_.size(parsedPathname), 1)) as Hex;
  ...
  return (
    <HStack spacing={6} pl={6}>
      {avatar && <Avatar src={avatar} height='100px' w='75px' borderRadius='md' />}
      ...
      <Stack>
        ...
        <Skeleton isLoaded={!wearerLoading}>
          {!!_.get(firstCreated, 'createdAt') && (
            <Text>
              Hat wearer since:{' '}
              {_.get(firstCreated, 'createdAt') && format(Number(_.get(firstCreated, 'createdAt')) * 1000, 'MMMM yyyy')}
            </Text>
          )}
        </Skeleton>
      </Stack>
