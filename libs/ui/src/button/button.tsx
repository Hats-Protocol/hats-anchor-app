  ...
  'focus-visible:ring-ring inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        destructive: 'bg-destructive text-destructive-foreground hover:bg-destructive/70 shadow-sm',
        'destructive-outline':
          'border-destructive text-destructive hover:bg-destructive/10 border bg-transparent shadow-sm',
        outline: 'border-input bg-background/80 hover:bg-accent hover:text-accent-foreground border shadow-sm',
        'outline-blue':
          'bg-background/80 hover:bg-functional-link-primary/10 border-functional-link-primary text-functional-link-primary border shadow',
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, rounded, type = 'button', asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : 'button';
    return (
      <Comp className={cn(buttonVariants({ variant, size, className, rounded }))} type={type} ref={ref} {...props} />
    );
  },
);
