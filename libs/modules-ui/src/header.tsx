...
import { Badge, Box, Button, Flex, Heading, HStack, Icon, Skeleton, Stack, Text, Tooltip } from '@chakra-ui/react';
    ...
    <Stack pb={2} w='full'>
      <Box width='100%'>
        ...
        <Flex position='relative' justify='center'>
          <Skeleton isLoaded={!isHatDetailsLoading} h='40px' position='absolute' top='-10px'>
            <HStack>
              {/* {isCurrentWearer && <Badge colorScheme='green'>My Hat</Badge>} */}

              <Badge colorScheme={mutableStatus === MUTABILITY.MUTABLE ? 'blue' : 'red'} boxShadow='sm'>
                {mutableStatus}
              </Badge>

              <Badge colorScheme={activeStatus === STATUS.ACTIVE ? 'green' : 'red'} boxShadow='sm'>
                {activeStatus}
              </Badge>
      ...
      <Stack w='full' px={4} pt={10}>
        <Flex w='full' justify='space-between' align='baseline'>
          <Tooltip label={name || selectedHat?.details}>
            <Skeleton isLoaded={!isHatDetailsLoading} h={8} minW='100px' borderRadius='sm'>
              <Heading noOfLines={{ base: 2, md: 1 }}>{name || selectedHat?.details || '-'}</Heading>
            </Skeleton>
          </Tooltip>

          <Skeleton isLoaded={!isHatDetailsLoading} minW='50px'>
            <Button variant='link' color='blue.500' onClick={onCopy} rightIcon={<Icon as={CopyHash} boxSize={4} />}>
