    ...
    <Tabs w='100%'>
      <TabList overflow='scroll'>
        {_.map(modules, (m: ModuleDetails) => (
          <Tab key={m.id} minW='140px' h='50px' _selected={{ bg: 'blue.100' }}>
            {m.name
              ...
              <Accordion title='Details' open>
                <Stack>
                  ...
                  <HStack>
                    ...
                    {_.get(m, 'type.eligibility') && <Text>Eligibility</Text>}
                  ...
                  <HStack>
                    ...
                    <ChakraNextLink
                      href={`${explorerUrl(
                        _.get(m, 'deployments[0].chainId') as unknown as number,
                ...
                <UnorderedList spacing={1}>
                  {_.map(m.deployments, (d) => (
                    <ListItem key={d.chainId}>
                      ...
                      >
                        <Text>
                          {chainsMap(_.toNumber(d.chainId))?.name} - ({d.block})
                        </Text>
                      </ChakraNextLink>
                    </ListItem>
                  ))}
                ...
                <Accordion title='Roles' open>
                  <Stack>
                    {_.map(m.customRoles, (r) => {
                      const functionsForRole = _.filter(m.writeFunctions, (f) =>
                        _.includes(f.roles, r.id),
                      );
                      return (
                        <Stack key={r.name}>
                          <HStack spacing={1}>
                            <Text>{r.name}</Text>
                            <Text fontFamily='monospace'>({r.criteria})</Text>
                          </HStack>
