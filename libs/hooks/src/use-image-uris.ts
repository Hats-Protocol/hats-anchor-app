...
const useImageURIs = ({
  ...
  const onlyOnchainHats = useMemo(() => {
    if (onchainHats) {
      return _.filter(hats, (hat: AppHat) => _.includes(_.map(onchainHats, 'id'), hat?.id));
    }
    return hats;
  }, [hats, onchainHats]);
  ...
  const { data: imagesData, isLoading: imagesLoading } = useQuery({
    ...
    queryKey: ['imageURIs', { hatIds: _.map(onlyOnchainHats, 'id'), chainIds, onchain }],
    queryFn: () => {
      const clients: any[] = _.map(chainIds, (cId: number) => viemPublicClient(cId));
  ...
  const uniqueImageUris = useMemo(() => {
    return _.filter(_.uniq(_.map(hatImageUris, 'result')) as string[], (img: string) => img !== '');
  }, [hatImageUris]);
  ...
  const mergedWithHats = useMemo(() => {
    ...
    return _.map(_.flatten(onlyOnchainHats), (hat: any, i: any) => {
      const imageIndex = _.findIndex(uniqueImageUris, (img: any) => img === _.get(_.nth(hatImageUris, i), 'result'));
  ...
  }, [hatImageUris, onlyOnchainHats, uniqueImageUris, imageUrls, imagesLoading, isLoaded, initialHatsData]);
