...
export function treeCreateEventIdToTreeId(id) {
  const hexString = id.toHexString();
  const normalizedHexString = hexString.replace(/^0x/, '');
  const paddingLength = 64 - normalizedHexString.length;
  const paddedHexString = '0'.repeat(paddingLength) + normalizedHexString;
  const mainHatIdHex = paddedHexString.slice(0, 8);
  return parseInt(`0x${mainHatIdHex}`, 16);
}
