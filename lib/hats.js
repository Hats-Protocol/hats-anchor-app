...
export function toTreeStructure(data, hatIdToImage) {
  if (data === undefined || data.hats === undefined) {
    return [];
  }

  const hatsArray = [];
  for (const hat of data.hats) {
    // recursion starting point is a dummy hat
    ...
    for (const linkedTree of hat.linkedTrees) {
      hatsArray.push({
        hatName: linkedTree.id,
        hatParent: hat.prettyId,
        imageURI: hatIdToImage[prettyIdToId(linkedTree.id)],
        linked: true,
      });
    }
  }
  ...
  // if top-hat is linked to another hat, add it to the tree
  if (data?.linkedToHat !== null) {
    hatsArray.push({
      hatName: data.linkedToHat.prettyId,
      hatParent: 'dummy',
      imageURI: hatIdToImage[prettyIdToId(data.linkedToHat.prettyId)],
      linked: true,
    });
  }
  hatsArray.push({ hatName: 'dummy', hatParent: 'null' });

  return arrayToTreeRecursive(hatsArray, 'dummy');
}
